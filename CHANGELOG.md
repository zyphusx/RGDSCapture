...
- **New app icon.** The placeholder wordmark is gone, replaced by the RG DS
  ...
  set into the lower one — on a dark rounded tile. It is drawn as three shapes
  and no lettering so it stays readable at the 16 px taskbar and title bar
  size, where the old "RGDSC" text was an unreadable smudge. All eight sizes
  from 16 to 256 px are still there, but the 256 entry is now PNG-compressed,
  which takes the file from 401 KB to 164 KB.
- The icon is generated by `tools/generate_icon.py` rather than hand-drawn,
  so it can be rebuilt from source on any machine with Python and Pillow —
  no Windows-only image editor anywhere in the pipeline. See
  [Regenerating the App Icon](README.md#regenerating-the-app-icon-optional).
